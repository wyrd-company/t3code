...
Repository variables named for any of the six public configuration values are optional overrides for deliberate divergence; none are required. Any variable left set shadows derivation. The release logs a warning that names the variable, upstream value, and override value when they differ. The built bundle must contain a non-empty value for every field and match the upstream package unless an override is explicitly captured before derivation.
